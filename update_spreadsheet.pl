...
#$editor->writecell($ss_field_to_letter{'rem_date'} . $$email_to_row_hashref{$email}, $$data_hashref{'remaining_data'});
...
sub get_account_data {
	...
	#Fetch account information for account.
	$mech->get($base_url . "/my-account/account-information?a=" . $account_number);
	sleep(1);
	my $account_info = $mech->content( decoded_by_headers => 1 );
	unless ($account_info =~ m/Airtime Exp Date/){
		die "Account information page failed to load for account $username\n" if $login_attempt_num > 2;
		$login_attempt_num++;
		return get_account_data($username, $account_pass, $login_attempt_num);
	}
	$phone_fields{'plan_type'} = $1 if $account_info =~ m/"PlanName":"([^"]+)"/;
	$phone_fields{'account_balance'} = $1 if $account_info =~ m/"AccountBalance":"(\$\d{1,2}\.\d{2})"/;
	#Formatted as MM/DD/YYYY.
	$phone_fields{'plan_expire_date'} = "$2/$3/$1" if $account_info =~ m/"AirtimeExpDate":"(\d{4})-(\d{2})-(\d{2})[^"]+"/;
	#Grab kb data, divide to turn to gb, round to 2 decimals.
	$phone_fields{'remaining_data'} = sprintf("%.2f GB", $1/1e6) if $account_info =~ m/"Data":"(\d+)"/;
